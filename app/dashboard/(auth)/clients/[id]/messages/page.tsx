...
import { Send, Paperclip, MessageSquare, Mail, Smartphone, PhoneCall } from "lucide-react";

type ComposableChannel = Exclude<CommChannel, "voice">;
type ViewFilter = "all" | CommChannel;

const filterTabs: { value: ViewFilter; label: string; icon: React.ElementType }[] = [
  { value: "all", label: "All", icon: MessageSquare },
  { value: "portal", label: "Portal", icon: MessageSquare },
...
export default function ClientMessagesPage() {
  ...
  const filteredThread = viewFilter === "all"
    ? thread
    : thread.filter(m => m.channel === viewFilter);
    ...
    <div className="flex flex-col" style={{ height: "calc(100vh - 320px)" }}>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                !hasMessages && tab.value !== "all" && "opacity-40"
              )}
            >
              <Icon className="size-3" />
              {tab.label}
              {hasMessages && tab.value !== "all" && (
                <span className={cn(
                  "min-w-[1rem] rounded-full px-1 text-center text-[9px] tabular-nums leading-tight",
                  isActive ? "bg-foreground/10" : "text-muted-foreground/50"
                )}>
                  {count}
                </span>
              )}
      ...
      <div className="shrink-0 border-t border-border/50 pt-3 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {suggestSms && composeChannel !== "sms" && (
            <span className="text-[9px] text-amber-600">
              No portal login in {daysSinceLogin === Infinity ? "ever" : `${daysSinceLogin}d`}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            placeholder={channelPlaceholders[composeChannel]}
            value={input}
