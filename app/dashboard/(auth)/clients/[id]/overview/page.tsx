...
export default function ClientOverviewPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Document Extraction - core workflow */}
        ...
        <div className="space-y-2">
          ...
          {clientCompliance.map(a => (
            <div key={a.id} className="flex items-start gap-2 rounded-xl border p-3">
              ...
              <div><div className="text-sm font-semibold">{a.title}</div><p className="mt-0.5 text-xs text-muted-foreground">{a.description}</p><div className="mt-1 text-xs text-red-600">Fine risk: {a.fineRisk}</div></div>
            </div>
          ))}
          {clientAnomalies.map(a => (
            <div key={a.id} className="flex items-start gap-2 rounded-xl border p-3">
              <AlertTriangle className="mt-0.5 size-4 shrink-0 text-amber-500" />
              <div><div className="text-sm font-semibold">{a.metric}: {a.changePercent}% change</div><p className="mt-0.5 text-xs text-muted-foreground">{a.aiExplanation}</p></div>
            </div>
          ))}
          {clientDeductions.map(a => (
            <div key={a.id} className="flex items-start gap-2 rounded-xl border p-3">
              <CheckCircle className="mt-0.5 size-4 shrink-0 text-emerald-500" />
              <div><div className="text-sm font-semibold">{a.deductionType} ({a.section})</div><p className="mt-0.5 text-xs text-muted-foreground">~${a.estimatedSavings.toLocaleString()} estimated savings</p></div>
            </div>
          ))}
          {clientExtensions.map(a => (
            <div key={a.id} className="flex items-start gap-2 rounded-xl border p-3">
              <Clock className="mt-0.5 size-4 shrink-0 text-amber-500" />
              <div><div className="text-sm font-semibold">Extension likelihood: {a.probability}%</div><p className="mt-0.5 text-xs text-muted-foreground">{a.factors.join(", ")}</p></div>
            </div>
          ))}
        </div>
      )}
