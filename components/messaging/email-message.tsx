    ...
    <div className="space-y-2" onClick={(e) => e.stopPropagation()}>
      {message.emailSubject && (
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-foreground/90">{message.emailSubject}</span>
        </div>
      )}

      <p className="text-sm leading-relaxed text-foreground/80">{displayContent}</p>
