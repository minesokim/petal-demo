...
/**
 * Tax Questions — 1:1 port of tax-questions.jsx. 7 yes/no toggle
 * cards with dedicated inline icons per question. The foreign
 * accounts toggle uses the `emphasis` variant (rust instead of ink
 * when on) because it triggers FBAR reporting.
 */
...
export function ScreenTaxQuestions({ t }: { t: PortalTheme }) {
  const { taxQuestions, patch, goNext, goPrev } = useIntakeStore();
  const toggle = (k: keyof typeof taxQuestions) =>
    patch({ taxQuestions: { ...taxQuestions, [k]: !taxQuestions[k] } });

  const questions: {
    k: keyof typeof taxQuestions;
      ...
      <div
        ...
        <Stack gap={10} style={{ padding: "22px 24px 16px", flex: 1 }}>
          ...
          <div style={{ marginTop: 10 }}>
            <AntonioNote t={t}>
              The digital assets question is on the front page of the 1040 now.
              The IRS is watching this closely. Foreign accounts and crypto are
              audit magnets — better to disclose than hide.
            </AntonioNote>
          </div>
        </Stack>
