...
import { motion, useInView, useAnimation, useSpring } from "motion/react";
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <HeaderElement
            ...
            className={cn(
              ...
              onActionClick && "group rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            )}
          >
              ...
              {chartData.map((item, index) => (
                <div key={item.name} className="flex h-full flex-1 flex-col items-center justify-end gap-1">
                  <motion.div
                    className={cn(
                      "w-full rounded-t-lg",
                      item.color ? item.color : index === chartData.length - 1 ? highlightedBarColor : defaultBarColor
                    )}
