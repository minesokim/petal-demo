...
| 1:45 | "Here's the part auditors love. The K-1 from the S-corp flows to the personal return — Petal drew that relationship. And the workpaper traces *every line on the return back to the run, the workpaper, and the source document.*" | On the **Overview tab** (scroll down): point at the **Relationships** card ("David & Grace Park (1040) ← K-1 ← Park Family Dental (1120S, 100%)") and the **workpaper block** — click a line's *View run*. |
| 2:05 | "And every return carries its own authorizations — the engagement letter, the §7216 consent, the 8879 e-file authorization — nothing transmitted until it's signed and I've approved." | Open a **Park return** → the **Organizer** card on Overview → the **Authorizations** row (engagement letter · §7216 · 8879 status). |
...
> **Deep-dive note:** on the client record the returns table, the K-1 **Relationships** card, and the **workpapers** all live on **Overview**; the **Intake** tab holds the readiness/onboarding card; the **More ▾** dropdown holds Billing, Memory, Positions, Activity. Per-return e-file authorizations live in each return's **Organizer** card (Overview). Deep-link Positions directly if you'd rather not open the menu — the at-risk link and chat both jump there.
