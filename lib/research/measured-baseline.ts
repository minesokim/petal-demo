...
export const MEASURED_BASELINE: MeasuredRun[] = [
  ...
  { set: "generated", description: "honest all-10-areas verified benchmark (50 cases). TRAJECTORY: 12/50 (24%) raw baseline → 19/50 (38%) after distilling the empty areas → 33/50 (66%) after the BM25-like body-text retrieval fix (score every chunk by query-term overlap in its BODY, weight 0.4 capped — surfaces the on-point distilled chunk over tangential keyword matches in crowded areas, moat-safe: recall-gate + entity 28/28 stay green). Remaining 17: 3 multistate (need CA R&TC source), 2 regs (§1.752-2/§1.707-3), subsection precision (§280A(g)/§163(h)), + a few retrieved-but-ungrounded. Goal: 90%.", model: "gpt-5.5 (codex)", config: "--set generated", total: 50, pass: 33, errorRatePct: 34.0, date: "2026-06-27" },
