...
export const MEASURED_BASELINE: MeasuredRun[] = [
  ...
  { set: "generated", description: "honest all-10-areas verified benchmark (50 cases). TRAJECTORY: 12/50 (24%) raw baseline → 19/50 (38%) after distilling the empty areas → 33/50 (66%) after the BM25-like body-text retrieval fix → 34/50 (68%) after the §163-grandfather/§3121 enrichment + §1.752-2/§1.707-3 regs. THE BOTTLENECK MOVED: retrieval is now strong (43/50 of the answer's authority is reachable) but only 36 answer — ~7 cases RETRIEVE the right authority and still ABSTAIN (the retrieve-then-extract / calibration zone = the gated moat-touching step). Remaining 16 fails = 4 multistate (non-IRC: PL 86-272 / Multistate Compact / Cal. R&TC — sourceable, allowed) + ~7 retrieved-but-abstained (gated) + 2 wrong-cite + §280A/§512 conceptual + §30D terminated. The allowed levers cap ~86%; crossing to 90% needs the gated extraction step.", model: "gpt-5.5 (codex)", config: "--set generated", total: 50, pass: 34, errorRatePct: 32.0, date: "2026-06-27" },
