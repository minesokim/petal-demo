...
export const MEASURED_BASELINE: MeasuredRun[] = [
  ...
  { set: "entity", description: "entity + capital-gains + depreciation/deduction + equity-comp (§83/§409A/ISO) source-verified set (27 cases); only the §351 control-80% codex abstain fails (model-robustness — grounds on Claude, so ~27/27 on the prod model)", model: "gpt-5.5 (codex)", config: "--set entity --fetch", total: 27, pass: 26, errorRatePct: 3.7, date: "2026-06-27" },
...
export const RELEASE_GATE = {
  ...
  entity: { floor: 25, of: 27, note: "entity + capital-gains + depreciation/deduction + equity-comp settled law — codex measured 26/27; floor 25 tolerates the known §351 codex abstain + one run-to-run flake. Raise as codex robustness improves; never lower it." },
} as const;
